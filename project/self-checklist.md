...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку | ✅ | `src/service/predictor.py` грузит `artifacts/model.joblib`; модель — GradientBoosting |
| 3  | Есть EDA и хотя бы один эксперимент с метриками | ✅ | `notebooks/01_eda.ipynb`; `report.md` разд. 3 и 5 |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам** | ✅ | LogisticRegression vs RandomForest vs GradientBoosting; `artifacts/model_comparison.csv`, `report.md` разд. 5 |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/` | ✅ | `src/data/`, `src/features/`, `src/models/`, `src/service/`, `src/utils/` |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker | ✅ | `Dockerfile`, `docker-compose.yml`; также инструкция без Docker в `README.md` разд. 4 |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов | ✅ | `configs/.env.example`, `SECURITY.md`, `.gitignore` исключает `.env` |
| 8  | Реализованы логи/наблюдаемость (консольные логи + `/health`) | ✅ | `src/utils/logging.py`, `src/service/metrics.py`, эндпоинты `/health` и `/metrics` |
| 9  | В `report.md` **обоснован выбор финальной модели** по экспериментам | ✅ | `report.md` разд. 5 — выбор GradientBoosting с разбором trade-off |
| 10 | `README.md` и `report.md` позволяют понять сценарий демонстрации | ✅ | `README.md` разд. 7, `report.md` разд. 9 |
